...
const dvcs_commands = {
    "/api/device/data": function(data) {
        let dvc = clients_conn[data.device];
        dvc.write(`{color=${data.RGB[0]},${data.RGB[1]},${data.RGB[2]}}`);
        dvc.write(`{state=${data.state}}`);
        dvc.write(`{mode=${data.mode}}`);
        dvc.write(`{params=${data.params}}`);
    }
}
